...
const PUBLIC_PATH = path.join(process.cwd(), 'public', 'semesters')
...
function createDirectoryStructure() {
  ...
  subjects.forEach(subject => {
    const subjectPath = path.join(PUBLIC_PATH, subject.semesterId.toString(), subject.id.toString())
    ...
    directories.forEach(dir => {
      const dirPath = path.join(subjectPath, dir)
    ...
    const moduleDirs = ['notes', 'presentations']
    moduleDirs.forEach(dir => {
      for (let i = 1; i <= 5; i++) {
        const modulePath = path.join(subjectPath, dir, `module-${i}`)
...
function createPlaceholderFiles() {
  subjects.forEach(subject => {
    ...
    // TLEP
    const tlepPath = path.join(subjectPath, 'tlep', 'tlep.pdf')
    ...
    // Notes
    for (let i = 1; i <= 5; i++) {
      const notesPath = path.join(subjectPath, 'notes', `module-${i}`, 'notes.pdf')
    ...
    // Presentations
    for (let i = 1; i <= 5; i++) {
      const pptPath = path.join(subjectPath, 'presentations', `module-${i}`, 'slides.pptx')
    ...
    // Activities
    const activity1Path = path.join(subjectPath, 'activity-1')
    ...
    const activity2Path = path.join(subjectPath, 'activity-2')
    ...
    // Papers
    const papersPath = path.join(subjectPath, 'papers', '2023.pdf')
